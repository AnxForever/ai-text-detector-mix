...
# 璁剧疆鐜鍙橀噺纭繚 UTF-8 缂栫爜
os.environ['PYTHONIOENCODING'] = 'utf-8'

# 璁剧疆鏍囧噯杈撳嚭缂栫爜涓篣TF-8锛堟洿鍋ュ．鐨勬柟寮忥級
if sys.platform == 'win32':
    try:
        sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8', errors='replace')
        sys.stderr = io.TextIOWrapper(sys.stderr.buffer, encoding='utf-8', errors='replace')
    except AttributeError:
        # 濡傛灉 buffer 涓嶅彲鐢紝浣跨敤 reconfigure
        try:
...
clients = {"custom": True, "deepseek": True, "claude": True, "qwen": True, "glm": True}
...
class IntelligentCombinationGenerator:
    ...
    def _generate_prompt(self, attribute, topic, genre, role, style, constraint):
        ...
        # 鏍煎紡鍙樺紓锛?0% Markdown锛?0% 绾枃鏈紝40% 涓嶉檺鍒?
        format_instruction = random.choices(
            ["璇蜂娇鐢∕arkdown鏍煎紡缁勭粐鍐呭锛屽寘鎷爣棰樸€佸垪琛ㄣ€佸姞绮楃瓑銆?,
             "璇蜂娇鐢ㄧ函鏂囨湰鏍煎紡锛屼笉瑕佷娇鐢ㄤ换浣昅arkdown鏍囪锛堝#銆?銆?绛夛級銆?,
             ""],
            weights=[0.3, 0.3, 0.4]
        )[0]
        ...
        if format_instruction:
            parts.append(f"銆愭牸寮忚姹傘€憑format_instruction}")
...
def call_api(model_name: str, prompt: str, max_tokens: int = 2000) -> Optional[str]:
    ...
    actual_model = MODEL_CONFIGS.get(model_name, {}).get("model_name", "deepseek-v3.2-chat")
    data = {
        "model": actual_model,
        "messages": [{"role": "user", "content": prompt}],
        "max_tokens": max_tokens,
...
def main_auto():
    ...
    # 閰嶇疆鐢熸垚鍙傛暟锛堟柊鏂规 - 浠呬娇鐢ㄥ彲鐢ㄦā鍨嬶級
    SAMPLES_PER_MODEL = {
        "deepseek": 10000,   # DeepSeek V3.2 (涓诲姏)
        "qwen": 8000,        # Qwen Max
        "claude": 7000       # Claude Sonnet 4.5
    }
    ACTIVE_MODELS = ["deepseek", "qwen", "claude"]
    ...
    # 鐢熸垚缁勫悎璁″垝锛堟寜鏈€澶ч渶姹傜敓鎴愶級
    plan_file = os.path.join(OUTPUT_DIR, "auto_generation_plan.json")
    max_samples = max(SAMPLES_PER_MODEL.values())
    ...
    for model_name in ACTIVE_MODELS:
        ...
        print(f"\n{'=' * 50}", flush=True)
        print(f"寮€濮嬩负妯″瀷 {model_name.upper()} 鐢熸垚鏁版嵁", flush=True)
        ...
        model_samples = SAMPLES_PER_MODEL.get(model_name, 800)
        for idx, plan_item in enumerate(ungenerated_plans[:model_samples]):
            print(f"[{model_name.upper()} {idx + 1}/{min(len(ungenerated_plans), model_samples)}] "
                  f"璐ㄩ噺:{plan_item['quality_score']:.2f} 灞炴€?{plan_item['attribute']} 涓婚:{plan_item['topic'][:15]}...", flush=True)
            ...
            if text:
                ...
                print(f"    {quality_str} ({len(text)} 瀛楃)", flush=True)
            else:
                failed_count += 1
                print(f"    鉂?澶辫触", flush=True)

            # 姣忕敓鎴?0鏉℃垨缁撴潫鏃讹紝淇濆瓨涓€娆¤繘搴?
            if (idx + 1) % 20 == 0 or (idx + 1) == min(len(ungenerated_plans), model_samples):
                # 淇濆瓨鏁版嵁
                if data_records:
