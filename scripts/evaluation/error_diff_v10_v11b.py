#!/usr/bin/env python3
"""
V11b Error Diff Analysis - 对比 V10 vs V11b 的逐样本预测差异

输出:
  - 新增错误 (V10 correct, V11b wrong)
  - 修复错误 (V10 wrong, V11b correct)
  - 按长度桶和标签分组统计
"""
...
def main():
    ...
    models_info = {
        ...
        "V11b": ROOT / "models" / "bert_v11b_augmented",
    }
    ...
    for eval_name, eval_path in EVAL_SETS:
        ...
        preds_v11b, confs_v11b = predict(loaded["V11b"][1], loaded["V11b"][0], texts, labels, device)

        v10_correct = preds_v10 == labels
        v11b_correct = preds_v11b == labels

        # New errors: V10 correct, V11b wrong
        new_errors = v10_correct & ~v11b_correct
        # Fixed: V10 wrong, V11b correct
        fixed = ~v10_correct & v11b_correct
        # Both wrong
        both_wrong = ~v10_correct & ~v11b_correct

        print(f"\n  V10 accuracy:  {v10_correct.mean()*100:.2f}% ({v10_correct.sum()}/{len(labels)})")
        print(f"  V11b accuracy: {v11b_correct.mean()*100:.2f}% ({v11b_correct.sum()}/{len(labels)})")
        print(f"\n  New errors (V10 ok -> V11b wrong): {new_errors.sum()}")
        print(f"  Fixed errors (V10 wrong -> V11b ok): {fixed.sum()}")
        print(f"  Both wrong: {both_wrong.sum()}")
        print(f"  Net change: {fixed.sum() - new_errors.sum():+d}")

        set_report = {
            "v10_accuracy": round(v10_correct.mean() * 100, 2),
            "v11b_accuracy": round(v11b_correct.mean() * 100, 2),
        ...
        if new_errors.sum() > 0:
            ...
            for idx in np.where(new_errors)[0]:
                ...
                pred_str = "AI" if preds_v11b[idx] == 1 else "Human"
                ...
                conf = confs_v11b[idx]
                print(f"    [{idx}] len={lengths[idx]} bin={lb} label={label_str} pred={pred_str} conf={conf:.4f} source={source}")
        ...
        # Length bin summary
        print(f"\n  --- Length Bin Accuracy ---")
        print(f"  {'Bin':<12} {'Count':>6} {'V10 Acc':>8} {'V11b Acc':>9} {'Delta':>7}")
        print(f"  {'-'*12} {'-'*6} {'-'*8} {'-'*9} {'-'*7}")
        bin_stats = []
        for bn, (lo, hi) in zip(BIN_NAMES, LENGTH_BINS):
            ...
            v11b_acc_bin = v11b_correct[mask].mean() * 100
            delta = v11b_acc_bin - v10_acc_bin
            print(f"  {bn:<12} {mask.sum():>6} {v10_acc_bin:>7.2f}% {v11b_acc_bin:>8.2f}% {delta:>+6.2f}")
            bin_stats.append({
                "bin": bn,
                "count": int(mask.sum()),
                "v10_acc": round(v10_acc_bin, 2),
                "v11b_acc": round(v11b_acc_bin, 2),
                "delta": round(delta, 2),
            })
        ...
        for lab, lab_name in [(0, "Human"), (1, "AI")]:
            ...
            v11b_a = v11b_correct[mask].mean() * 100
            print(f"\n  {lab_name} (n={mask.sum()}): V10={v10_a:.2f}% V11b={v11b_a:.2f}% delta={v11b_a-v10_a:+.2f}")

        report[eval_name] = set_report

    # Save
    out_path = ROOT / "docs" / "plans" / "v11b_error_diff_analysis.json"
