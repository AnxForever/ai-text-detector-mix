...
# Load batch data
with open('datasets/mixed/hybrid/c2_batch.json') as f:
    c2_batch = json.load(f)
with open('datasets/mixed/hybrid/c3_batch.json') as f:
    c3_batch = json.load(f)
with open('datasets/mixed/hybrid/c4_batch.json') as f:
    c4_batch = json.load(f)

print(f"Loaded: C2={len(c2_batch)}, C3={len(c3_batch)}, C4={len(c4_batch)}")

# Load existing hybrid data
hybrid_df = pd.read_csv('datasets/mixed/hybrid/hybrid_dataset.csv')
...
combined_hybrid.to_csv('datasets/mixed/hybrid/hybrid_dataset_expanded.csv', index=False)
print("\nSaved to: datasets/mixed/hybrid/hybrid_dataset_expanded.csv")

# Recreate combined dataset
original_train = pd.read_csv('datasets/active/core_v1/train.csv')
original_val = pd.read_csv('datasets/active/core_v1/val.csv')
original_test = pd.read_csv('datasets/active/core_v1/test.csv')
...
# Save
train.to_csv('datasets/active/core_v1/train.csv', index=False)
val.to_csv('datasets/active/core_v1/val.csv', index=False)
test.to_csv('datasets/active/core_v1/test.csv', index=False)

# Create hybrid-only test
hybrid_test = test[test['category'].isin(['C2', 'C3', 'C4', 'Human'])]
hybrid_test.to_csv('datasets/mixed/hybrid/hybrid_dataset_with_sep.csv', index=False)

print(f"\nHybrid test: {len(hybrid_test)}")
print(hybrid_test['category'].value_counts())
print("\nSaved to: datasets/archive/combined_v2/")
