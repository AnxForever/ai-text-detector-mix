...
# === 保守参数 ===
BATCH_SIZE = 8          # 小批次，省内存
MAX_LEN = 256           # 短序列，省内存
EPOCHS = 3              # 少轮数，可以后续继续
LR = 2e-5
ACCUMULATION_STEPS = 4  # 梯度累积，等效 batch_size=32
...
def main():
    print("=" * 60)
    print("BERT Training - Paired Dataset (保守资源模式)")
    print("=" * 60)
    ...
    # === 加载数据 ===
    print("\n>>> 加载 paired_v2 数据集")
    train_df = pd.read_csv('datasets/paired/paired_v2_train.csv')
    val_df = pd.read_csv('datasets/paired/paired_v2_val.csv')
    ...
    if 'source' in train_df.columns:
        print(f"\n  数据来源分布:")
        for src, cnt in train_df['source'].value_counts().head(8).items():
            print(f"    {src}: {cnt}")
    ...
    output_dir = 'models/bert_v5_paired'
