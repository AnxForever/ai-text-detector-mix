...
基于 V9 配置 (V7 fine-tune)，使用 V10 增强数据集。
改进目标：
  1. 解决教育/技术类 AI 文本漏检 (FN)
  2. 提升短文本多样性，减少误判 (FP)

配置 (沿用 V9):
  - Base model: bert_v7_improved (fine-tune)
  - batch_size: 8, max_length: 256
  - learning_rate: 1e-5
  - label_smoothing: 0.05
  - length_aware_loss: 0.1
  - early stopping: patience=2
  - 训练数据: datasets/merged_v2/train_v10.csv (62,980 样本)
  - 验证数据: datasets/merged_v2/val.csv (7,475 样本)
...
CONFIG = {
    ...
    "train_data": str(ROOT / "datasets" / "merged_v2" / "train_v10.csv"),
    "val_data": str(ROOT / "datasets" / "merged_v2" / "val.csv"),
    "output": str(ROOT / "models" / "bert_v10_augmented"),
    "batch_size": 8,
    "max_length": 256,
    "epochs": 5,
    "learning_rate": 1e-5,
...
class LengthAwareLabelSmoothingLoss(nn.Module):
    """组合 Label Smoothing + Length-aware 权重"""
...
def load_and_clean(path, min_len, max_len):
    ...
    print(f"  {path}: {before} → {len(df)} (清洗移除 {before - len(df)})")
    return df
...
def main():
    ...
    # V10 新增来源统计
    v10_sources = train_df[train_df['source'].str.startswith('v10_', na=False)]
    if len(v10_sources) > 0:
        print(f"\n  V10 新增样本: {len(v10_sources)}")
        for src, cnt in v10_sources['source'].value_counts().items():
            print(f"    {src}: {cnt}")
    ...
    for epoch in range(cfg['epochs']):
        ...
        if val_acc > best_acc:
            ...
            patience_counter += 1
            print(f"  Val Acc 未提升 ({patience_counter}/{cfg['patience']})")
    ...
    log = {
        'version': 'v10',
        'strategy': 'scheme_alpha_data_augmentation',
        'base_model': cfg['base_model'],
        'config': cfg,
        'train_samples': len(train_df),
        'val_samples': len(val_df),
        'v10_new_samples': len(v10_sources),
