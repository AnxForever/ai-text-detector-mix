...
控制变量设计：
  - Base model: bert_v7_improved (与 V10 相同)
  - 所有超参、损失函数、采样策略均沿用 V10
  - 目的: 隔离"数据清洗"这一单一变量的影响

用法:
    python scripts/training/train_v11a.py
"""
import os
import sys
import time
...
CONFIG = {
    ...
    "train_data": str(ROOT / "datasets" / "merged_v2" / "train_v11_candidate.csv"),
    "val_data": str(ROOT / "datasets" / "merged_v2" / "val.csv"),
    "output": str(ROOT / "models" / "bert_v11a_clean"),
    "batch_size": 8,
    "max_length": 256,
    "epochs": 5,
    "learning_rate": 1e-5,
...
def main():
    ...
    print("=" * 70)
    print("  V11a BERT Training - Cleaned data baseline (no weak-domain supplement)")
    print("=" * 70)
    ...
    log = {
        'version': 'v11a',
        'strategy': 'data_cleaning_baseline',
        'description': 'V10 config + cleaned train data (hard pattern + unapproved unknown removed)',
