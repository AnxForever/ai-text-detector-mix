...
控制变量设计：
  - Base model: bert_v7_improved (与 V10/V11a 相同)
  - 所有超参、损失函数、采样策略均沿用 V10
  - 目的: 在 V11a 基础上隔离"弱域补充"这一单一变量的影响

用法:
    python scripts/training/train_v11b.py
"""
import os
import sys
import time
...
CONFIG = {
    ...
    "train_data": str(ROOT / "datasets" / "merged_v2" / "train_v11b_candidate.csv"),
    "val_data": str(ROOT / "datasets" / "merged_v2" / "val.csv"),
    "output": str(ROOT / "models" / "bert_v11b_augmented"),
    "batch_size": 8,
    "max_length": 256,
    "epochs": 5,
    "learning_rate": 1e-5,
...
def main():
    ...
    print("=" * 70)
    print("  V11b BERT Training - Cleaned data + weak-domain supplement (+600)")
    print("=" * 70)
    ...
    log = {
        'version': 'v11b',
        'strategy': 'data_cleaning_plus_weak_domain_supplement',
        'description': 'V10 config + cleaned train data + 600 weak-domain samples (formal_collected + LLaMA-405B)',
