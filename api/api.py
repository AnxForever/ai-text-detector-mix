...
class HybridTextDetector:
    def __init__(self):
        ...
        try:
            self.classifier_tokenizer = BertTokenizer.from_pretrained('models/bert_improved/best_model')
            self.classifier = BertForSequenceClassification.from_pretrained('models/bert_improved/best_model').to(self.device)
            self.classifier.eval()
            print("Classifier loaded (bert_improved/best_model).")
...
async def chat_completions(request: ChatRequest):
    ...
    # 从环境变量或配置读取API密钥
    api_key = os.getenv("OPENAI_API_KEY", "sk-WPzv2WwpDbnLp02uro0DyPUy0LyI3VjIRmngMj8fm7BLQqSq")
